...
clause("1") -->
	...
	poslit(advisedBy(A2,A1)),
	poslit(c1(A1,A2)).
poslit("1",advisedBy(_,_)).
poslit("1",c1(_,_)).
...
clause("2").
clause("2") -->
	insol(advisedBy(A4,A2)),
	neglit(advisedBy(A4,A2)),
	{
	 not tempAdvisedBy(A4,A2), taughtBy(A1,A2,A3),
	 courseLevel(A1,level500), ta(A1,A4,A3)
	},
	poslit(c2(A1,A2,A3,A4)).
neglit("2",advisedBy(_,_)).
poslit("2",c2(_,_,_,_)).
...
clause("6") -->
	insol(advisedBy(A2,A1)),
	neglit(advisedBy(A2,A1)),	
	{student(A1), not tempAdvisedBy(A1,A2)},
	poslit(c6(A1,A2)).
neglit("6",advisedBy(_,_)).
poslit("6",c6(_,_)).
...
clause("7") -->
	...
	poslit(advisedBy(A2,A1)),
	poslit(c7(A1,A2)).
neglit("7",advisedBy(_,_)).
poslit("7",c7(_,_)).

%0.671981  !advisedBy(a1,a1)
clause("8").
clause("8") -->
	insol(advisedBy(A1,A1)),
	neglit(advisedBy(A1,A1)),
	poslit(c8(A1)).
neglit("8",advisedBy(X,X)).
poslit("8",c8(_)).
...
clause("11") -->
	insol(advisedBy(A4,A2)),
	neglit(advisedBy(A4,A2)),
	{inPhase(A4,postquals), ta(A1,A4,A3),
	 not courseLevel(A1,level100),
	 taughtBy(A1,A2,A3)},
	poslit(c11(A1,A2,A3,A4)).
neglit("11",advisedBy(_,_)).
poslit("11",c11(_,_,_,_)).

% the following could be folded into clause 11 !!
%0.112133  !taughtBy(a1,a2,a3) v courseLevel(a1,"Level100") v !advisedBy(a4,a2) v !inPhase(a4,"PostQuals") v ta(a1,a4,a3)
clause("12").
clause("12") -->
	insol(advisedBy(A4,A2)),
	neglit(advisedBy(A4,A2)),
	{inPhase(A4,postquals),
	 taughtBy(A1,A2,A3), 
	 not courseLevel(A1,level100),
	 not ta(A1,A4,A3)
	},
	poslit(c12(A1,A2,A3,A4)).
neglit("12",advisedBy(_,_)).
poslit("12",c12(_,_,_,_)).
...
clause("13") -->
	...
	poslit(c13(A1,A2,A3,A4)).
neglit("13",advisedBy(_,_)).
poslit("13",c13(_,_,_,_)).
...
clause("14") -->
	insol(advisedBy(A4,A2)),
	neglit(advisedBy(A4,A2)),
	{inPhase(A4,postgenerals), ta(A1,A4,A3),
	 not courseLevel(A1,level100),
	 taughtBy(A1,A2,A3)},
	poslit(c14(A1,A2,A3,A4)).
neglit("14",advisedBy(_,_)).
poslit("14",c14(_,_,_,_)).
...
clause("15") -->
	...
	poslit(c15(A1,A2,A3,A4)).
neglit("15",advisedBy(_,_)).
poslit("15",c15(_,_,_,_)).
...
clause("16") -->
	...
	poslit(c16(A1,A2,A3,A4)).
neglit("16",advisedBy(_,_)).
poslit("16",c16(_,_,_,_)).
...
clause("17") -->
	...
	poslit(advisedBy(A2,A3)),
	poslit(advisedBy(A3,A2)),
	poslit(c17(A1,A2,A3)).
neglit("17",advisedBy(_,_)).
poslit("17",c17(_,_,_)).
